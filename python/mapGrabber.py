...
folder_path = f'/home/sam/Desktop/SNAPSHOT/GIS2/'
...
def download_nation_level_census_files():
    for web_name, my_file_name in nation_level_geography_dict.items():
        ...
        if not path.exists(file_path):
            ...
            download_path = f'https://www2.census.gov/geo/pvs/tiger2010st/tl_2010_us_{web_name}.zip';
            response = requests.get(download_path);
            z = zipfile.ZipFile(io.BytesIO(response.content));
            z.extractall(folder_path);
...
def rename_nation_level_census_files():
    for web_name, my_file_name in nation_level_geography_dict.items():
        old_path = folder_path + '/tl_2010_us_' + web_name;
...
def delete_fields(layer):
  L = [l for l in layer.fields().names() if l not in ['GEOID10','NAME10','ALAND10']]
  for f in L:
