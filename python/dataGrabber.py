...
def combineCSVs():
    group_list = pd.read_csv(f'/home/sam/{year}Variables.csv').Group.unique()
    for group in group_list:
        for i in range(1,100):
            file_path = f"/home/sam/{group}_{i}.csv"
            if path.exists(file_path):
                dfA = pd.read_csv(f"/home/sam/{group}_0.csv",dtype={f'GEOID{year[2:]}': object})
                dfB = pd.read_csv(f"/home/sam/{group}_{i}.csv",dtype={f'GEOID{year[2:]}': object})
...
year = "2000"
def main():
    national_geos = ["nation","state","metroSA","urban","zip","county",'place','region','division','uschool'];
    ...
    for group in group_list:
        print(int(group[1:4]))
        if int(group[1:4]) > 18:
            dfVar = pd.read_csv(f'/home/sam/{year}Variables.csv')
            ...
            for i in range(1+len(hoard)//50):
                ...
                df.to_csv(index=True,path_or_buf="/home/sam/" + f'{year[2:]}_'f'{group}{suffix}.csv');
